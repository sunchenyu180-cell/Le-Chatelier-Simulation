...
<head>
    ...
    <title>Le Chatelier's Principle Simulation</title>
...
<body class="bg-slate-100">
    ...
    <script type="text/babel">
        ...
        const Beaker = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4.5 3h15"/><path d="M6 3v16a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V3"/><path d="M14 9h4"/></svg>;
        const Cylinder = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5V19A9 3 0 0 0 21 19V5"/><path d="M3 12A9 3 0 0 0 21 12"/></svg>;
        const ThermometerSun = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 9a4 4 0 0 0-2 7.5"/><path d="M12 3v2"/><path d="M6.6 18.4l-1.4 1.4"/><path d="M20 4v10.54a4 4 0 1 1-4 0V4a2 2 0 0 1 4 0z"/><path d="M4 13H2"/><path d="M6.6 7.6L5.2 6.2"/></svg>;
        const Plus = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>;
        const Minus = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/></svg>;
        const Info = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>;
        const RefreshCw = ({size=24}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>;
        ...
        const ConcentrationSim = () => {
            const [Atot, setAtot] = useState(0.5);
            const [Btot, setBtot] = useState(0.5);
            ...
            const t = Math.min(1, Math.max(0, current_C / 1.0));
            const r = Math.round(250 - t * (250 - 140));
            const g = Math.round(230 - t * 230);
            const b = Math.round(100 - t * 100);
            const fluidStyle = { height: '75%', backgroundColor: `rgb(${r}, ${g}, ${b})` };
                ...
                <div className="flex flex-col md:flex-row gap-8 items-center justify-center p-6 bg-slate-50 rounded-xl">
                    <div className="relative w-40 h-56 flex flex-col items-center justify-end">
                        <div className="absolute top-0 w-36 h-full border-4 border-slate-300 rounded-b-[2.5rem] bg-white/50 backdrop-blur overflow-hidden flex items-end shadow-inner z-10">
                            <div className="w-full transition-colors duration-75" style={fluidStyle} />
                        </div>
                        <div className="absolute top-[-6px] w-40 h-4 border-4 border-slate-300 rounded-[50%] bg-white/20 z-20"></div>
                        <div className="absolute bottom-6 text-white font-bold drop-shadow-md z-30 opacity-80">Mixture</div>
                    </div>
        ...
        const PressureSim = () => {
            ...
            const current_nA = useLerpState(target_nA, 0.05);
                ...
                <div className="flex flex-col md:flex-row gap-8 items-center justify-center p-6 bg-slate-50 rounded-xl">
                    <div className="relative w-32 h-64 flex flex-col items-center">
                        <div className="absolute bottom-0 w-full h-56 border-x-4 border-b-4 border-slate-400 rounded-b-lg bg-white overflow-hidden shadow-inner flex flex-col justify-end">
                            <div className="w-full transition-colors duration-75 relative" style={gasContainerStyle}>
                                <div className="absolute inset-0 opacity-20" style={particlesStyle}></div>
                            </div>
                        </div>
                        <div className="absolute w-36 h-4 bg-slate-600 rounded-sm z-20 cursor-ns-resize shadow-md transition-all duration-75" style={pistonStyle}>
                    ...
                    <div className="flex-1 max-w-md space-y-6">
                        ...
                        <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                            ...
                            <input type="range" min="2" max="10" step="0.1" value={V} onChange={handleVolumeChange} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
        ...
        const TemperatureSim = () => {
            ...
            const Kc_T = useMemo(() => Kc298 * Math.exp((-dH / R) * (1 / T_kelvin - 1 / 298.15)), [T_kelvin, dH]);
            const target_nA = useMemo(() => calcPressureEq(V, N0, Kc_T), [V, N0, Kc_T]);
            const current_nA = useLerpState(target_nA, 0.02);
            ...
            return (
                <div className="flex flex-col md:flex-row gap-8 items-center justify-center p-6 bg-slate-50 rounded-xl">
                    <div className="relative w-48 h-56 flex flex-col items-center justify-end">
                        <div className="absolute bottom-0 w-full h-32 rounded-lg border-2 border-slate-300 transition-colors duration-300 z-0" style={bathBgStyle}>
                    ...
                    <div className="flex-1 max-w-md space-y-6 z-20">
                        ...
                        <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-100 shadow-sm">
                            ...
                            <input type="range" min="0" max="100" step="1" value={Tcelsius} onChange={handleTempChange} className={`w-full h-2 rounded-lg appearance-none cursor-pointer ${Tcelsius > 50 ? 'bg-red-200 accent-red-600' : Tcelsius < 15 ? 'bg-blue-200 accent-blue-600' : 'bg-slate-200 accent-slate-600'}`} />
                            ...
                            {tabs.map(tab => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex-1 min-w-[120px] py-4 px-6 flex items-center justify-center gap-2 text-sm font-semibold transition-colors ${isActive ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
                                    >
                                        <Icon size={18} />
                                        {tab.label}
                                    </button>
